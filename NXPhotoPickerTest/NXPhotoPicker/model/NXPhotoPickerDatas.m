...
@interface NXPhotoPickerDatas ()
@property (nonatomic , strong) ALAssetsLibrary *library;
@end

@implementation NXPhotoPickerDatas
...
- (void) getAllGroupAllPhotos:(BOOL)allPhotos withResource : (callBackBlock ) callBack{
    ...
    ALAssetsLibraryGroupsEnumerationResultsBlock resultBlock = ^(ALAssetsGroup *group, BOOL *stop){
        if (group) {
            ...
            // 包装一个模型来赋值
            NXPhotoPickerGroup *pickerGroup = [[NXPhotoPickerGroup alloc] init];
...
- (void) getGroupPhotosWithGroup : (NXPhotoPickerGroup *) pickerGroup finished : (callBackBlock ) callBack{
